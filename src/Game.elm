...
makeMove cellState coords model nextCmdFn =
    let
        ( gameState, winningLine ) =
            getGameState cellState modelWithMove

        modelWithMove =
            modelWithNewCellState cellState coords model
...
createWinningAnimation winningLine model =
    case winningLine of
        ...
        _ ->
            WinningAnimation.create winningLine model
...
modelWithNewCellState state coords model =
    let
        newModel =
            Board.setCellState state coords model
    in
...
getWinningColumn cellState wantedStates model =
    if getCellStatesForColumn 0 model == wantedStates then
        Just 0
...
getRowCoords rowIndex =
    [ ( 0, rowIndex ), ( 1, rowIndex ), ( 2, rowIndex ) ]


getColumCoords : Int -> List ( Int, Int )
getColumCoords columIndex =
    [ ( columIndex, 0 ), ( columIndex, 1 ), ( columIndex, 2 ) ]


getLeftToRightDiagonalCoords : List ( Int, Int )
getLeftToRightDiagonalCoords =
    [ ( 0, 0 ), ( 1, 1 ), ( 2, 2 ) ]


getRightToLeftDiagonalCoords : List ( Int, Int )
getRightToLeftDiagonalCoords =
    [ ( 0, 2 ), ( 1, 1 ), ( 2, 0 ) ]
...
modelWithEmptyBoard model =
    { model | board = Board.getInitialCells }
