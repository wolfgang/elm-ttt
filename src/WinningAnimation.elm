...
create winningLine model =
    let
        startCoords =
            ListExt.nth 0 winningLine ( -1, -1 )

        endCoords =
            ListExt.nth 2 winningLine ( -1, -1 )

        startRect =
            BoardUI.getCellRectAt startCoords model

        endRect =
            BoardUI.getCellRectAt endCoords model

        ( xOffset, yOffset ) =
            getEdgeOffsets (endRect.size / 2) startCoords endCoords
        ...
        ( startX, startY ) =
            ( startX0 + xOffset, startY0 + yOffset )

        ( endX, endY ) =
            ( endX0 + (endRect.size - xOffset), endY0 + (endRect.size - yOffset) )
    in
...
animate winningAnimation deltaSeconds =
    let
        ( currentX, currentY ) =
            winningAnimation.currentPoint

        ( endX, endY ) =
            winningAnimation.endPoint
        ...
        delta =
            winningAnimation.speed * deltaSeconds

        totalDistance =
            distanceSquared winningAnimation.startPoint winningAnimation.endPoint

        currentDistance =
            distanceSquared winningAnimation.currentPoint winningAnimation.startPoint
    in
...
getEdgeOffsets mult ( row0, col0 ) ( row2, col2 ) =
    if row0 == row2 && col0 /= col2 then
        ( mult, 0 )
    else if row0 /= row2 && col0 == col2 then
        ( 0, mult )
    else if ( row0, col0 ) == ( 0, 2 ) then
        ( 0, mult * 2 )
    else
        ( 0, 0 )
