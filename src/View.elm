    ...
    List.foldr
        (\cell rects ->
            if (cell.state == Empty) then
                ...
                rects ++ [ (drawOccupiedCell cell model) ]
        )
        []
        model.board
...
drawOccupiedCell cell model =
    let
        cellRect =
            BoardUI.getCellRectAt cell.coords model
...
drawXInCell cellRect model =
    let
        baseRect =
            rect 8 cellRect.size |> filled Color.black
    in
        group
            [ drawFormInCellRect
                cellRect
        ...
        Just coords ->
            let
                gs =
                    model.gridSettings
            in
                [ drawCellAt coords gs.cellHighlightColor model ]
...
drawCellAt ( row, col ) color model =
    let
        cellRect =
            BoardUI.getCellRectAt ( row, col ) model
    in
        drawFormInCellRect
            cellRect
...
toCollageCoordsWithOffset ( x, y ) ( offsetX, offsetY ) model =
    let
        gridSize =
            model.gridSettings.size
    in
        ( x - gridSize / 2 + offsetX, -(y - gridSize / 2 + offsetY) )


debugPrintAt pos string =
    let
        textWidth =
            (toFloat (String.length string) * 4)
    in
